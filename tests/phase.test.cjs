...
describe('phases list command', () => {
  ...
  test('lists phase directories sorted numerically', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '10-final'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '02-api'), { recursive: true });
  ...
  test('handles decimal phases in sort order', () => {
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '02-api'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '02.1-hotfix'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '02.2-patch'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '03-ui'), { recursive: true });
  ...
  test('--type plans lists only PLAN.md files', () => {
    const phaseDir = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '01-test');
  ...
  test('--phase filters to specific phase directory', () => {
    const phase01 = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '01-foundation');
    const phase02 = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '02-api');
...
describe('phase next-decimal command', () => {
  ...
  test('returns X.1 when no decimal phases exist', () => {
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '06-feature'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '07-next'), { recursive: true });
  ...
  test('increments from existing decimal phases', () => {
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '06-feature'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '06.1-hotfix'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '06.2-patch'), { recursive: true });
  ...
  test('handles gaps in decimal sequence', () => {
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '06-feature'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '06.1-first'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '06.3-third'), { recursive: true });
  ...
  test('returns error if base phase does not exist', () => {
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '01-start'), { recursive: true });
...
describe('phase-plan-index canonical format', () => {
  ...
  test('files_modified: underscore key is parsed correctly', () => {
    const phaseDir = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '04-ui');
    fs.mkdirSync(phaseDir, { recursive: true });
...
describe('phase add command', () => {
  ...
  test('adds phase after highest existing', () => {
    // With milestone v1.0 auto-detected, phase add uses 1.N milestone-prefix numbering
    fs.writeFileSync(
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.phase_number, '1.3', 'should be phase 1.3');
    assert.strictEqual(output.slug, 'user-dashboard');

    // Verify directory created
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '1.3-user-dashboard')),
      'directory should be created'
    );

    // Verify ROADMAP updated
    const roadmap = fs.readFileSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'ROADMAP.md'), 'utf-8');
    assert.ok(roadmap.includes('### Phase 1.3: User Dashboard'), 'roadmap should include new phase');
    assert.ok(roadmap.includes('**Depends on:** Phase 1.2'), 'should depend on previous');
  });

  test('handles empty roadmap', () => {
    // With milestone v1.0 auto-detected, first phase gets 1.1 (milestone-prefix numbering)
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'ROADMAP.md'),
      `# Roadmap v1.0\n`
    );
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.phase_number, '1.1', 'should be phase 1.1');
  });
...
`
    ...
    // Verify directory
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '01.1-fix-critical-bug')),
...
`
    ...
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '01-foundation'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '01.1-hotfix'), { recursive: true });
...
`
    ...
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '09.05-existing'), { recursive: true });
...
`
    ...
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '05-feature-work'), { recursive: true });
...
`
    ...
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '01-foundation'), { recursive: true });
    const p2 = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '02-auth');
    fs.mkdirSync(p2, { recursive: true });
    fs.writeFileSync(path.join(p2, '02-01-PLAN.md'), '# Plan');
    const p3 = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '03-features');
    ...
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '02-features')),
      'phase 3 should be renumbered to 02-features'
    );
    assert.ok(
      !fs.existsSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '03-features')),
      'old 03-features should not exist'
    );

    // Files inside should be renamed
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '02-features', '02-01-PLAN.md')),
      'plan file should be renumbered to 02-01'
    );
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '02-features', '02-02-PLAN.md')),
  ...
  test('removes decimal phase and renumbers siblings', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '06-main'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '6.1-fix-a'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '6.2-fix-b'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '6.3-fix-c'), { recursive: true });
    ...
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '6.2-fix-c')),
      '6.3 should be renumbered to 6.2'
    );
    assert.ok(
      !fs.existsSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '6.3-fix-c')),
  ...
  test('updates STATE.md phase count', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '01-a'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '02-b'), { recursive: true });
...
`
  ...
  test('detects last phase in milestone', () => {
    ...
    const p1 = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '01-only-phase');
    fs.mkdirSync(p1, { recursive: true });
    fs.writeFileSync(path.join(p1, '01-01-PLAN.md'), '# Plan');
    fs.writeFileSync(path.join(p1, '01-01-SUMMARY.md'), '# Summary');
...
`
    ...
    const p1 = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '01-auth');
    fs.mkdirSync(p1, { recursive: true });
    fs.writeFileSync(path.join(p1, '01-01-PLAN.md'), '# Plan');
    fs.writeFileSync(path.join(p1, '01-01-SUMMARY.md'), '# Summary');
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '02-api'), { recursive: true });
    ...
    const req = fs.readFileSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'REQUIREMENTS.md'), 'utf-8');
...
`
    ...
    const p1 = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '01-setup');
    fs.mkdirSync(p1, { recursive: true });
    fs.writeFileSync(path.join(p1, '01-01-PLAN.md'), '# Plan');
    fs.writeFileSync(path.join(p1, '01-01-SUMMARY.md'), '# Summary');
...
`
    ...
    fs.writeFileSync(path.join(p1, '01-01-SUMMARY.md'), '# Summary');
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '02-api'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '03-features'), { recursive: true });
...
`
    ...
    const p32 = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '03.2-ipsum');
    const p321 = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '03.2.1-dolor-sit');
    const p4 = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '04-amet');
...
describe('letter-suffix phase sorting', () => {
  ...
  test('lists letter-suffix phases in correct order', () => {
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '12-foundation'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '12.1-inserted'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '12A-split'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '12A.1-bugfix'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '12B-hotfix'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '13-deploy'), { recursive: true });
...
describe('is_last_phase roadmap fallback', () => {
  ...
  function setupPhase8Project(tmpDir, roadmapContent) {
    ...
    const p8 = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '08-path-architecture-foundation');
...
describe('phase complete milestone-scoped next-phase', () => {
  ...
  test('finds next phase within milestone, ignoring prior milestone dirs', () => {
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      const phaseDir = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', `${padded}-old-phase`);
      fs.mkdirSync(phaseDir, { recursive: true });
    ...
    const p5 = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '05-auth');
    ...
    fs.mkdirSync(path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '06-dashboard'), { recursive: true });
  ...
  test('detects last phase when only milestone phases are considered', () => {
    ...
    // getMilestonePhaseFilter reads root .planning/ROADMAP.md — write there too
