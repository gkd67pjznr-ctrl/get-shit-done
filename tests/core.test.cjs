 ...
 *
 * Tests for planningRoot, milestone-scoped core functions, and foundational
 * module exports including regressions for known bugs
 * (REG-01: loadConfig model_overrides, REG-02: getRoadmapPhaseInternal export).
 */
...
describe('normalizePhaseName', () => {
  ...
  test('handles decimal phases', () => {
    // Our fork: decimal phases are NOT zero-padded (2.1 stays 2.1, not 02.1)
    assert.strictEqual(normalizePhaseName('2.1'), '2.1');
  });

  test('handles multi-level decimals', () => {
    // Our fork: decimal phases are NOT zero-padded (1.2.3 stays 1.2.3, not 01.2.3)
    assert.strictEqual(normalizePhaseName('1.2.3'), '1.2.3');
  });
...
describe('getMilestoneInfo', () => {
  ...
  test('extracts version and name from roadmap', () => {
    ...
    // Our fork: getMilestoneInfo always returns 'milestone' for name
  ...
  test('returns active milestone when shipped milestone is collapsed in details block', () => {
    ...
    const info = getMilestoneInfo(tmpDir);
    // Our fork: simple first-match regex returns v0.1 (first in table)
    assert.strictEqual(info.version, 'v0.1');
    assert.strictEqual(info.name, 'milestone');
  });
...
describe('findPhaseInternal', () => {
  ...
  test('searches milestone phase dirs when not in flat layout', () => {
    // Our fork uses milestones/{version}/phases/{phase-dir} structure
    const msPhaseDir = path.join(tmpDir, '.planning', 'milestones', 'v1.0', 'phases', '01-foundation');
    fs.mkdirSync(msPhaseDir, { recursive: true });
    const result = findPhaseInternal(tmpDir, '1');
    assert.ok(result, 'should find phase in milestone dir');
    assert.strictEqual(result.milestone_scope, 'v1.0');
  });
});
