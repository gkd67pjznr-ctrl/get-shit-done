...
PLAN_SUGGESTIONS=$(echo "$INIT" | jq -c '.plan_suggestions // []')
TASK_PERFORMANCE=$(echo "$INIT" | jq -c '.task_performance // []')
```
...
```bash
PHASE_INFO=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" ${MILESTONE_FLAG})
```
...
```bash
PHASE_DESC=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" ${MILESTONE_FLAG} | jq -r '.section')
```
