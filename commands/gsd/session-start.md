...
description: Session briefing with GSD position, recent activity, pending suggestions, and skill budget
allowed-tools:
  - Read
  - Bash
  - Glob
...
# /gsd:session-start — Warm-Start Session Briefing
...
- Parse the JSON to extract:
  - `adaptive_learning.integration.suggest_on_session_start` (controls SESS-03)
  - `adaptive_learning.token_budget.max_percent` and `adaptive_learning.token_budget.warn_at_percent` (used in SESS-04)
...
If `adaptive_learning.integration.phase_transition_hooks` is `true` (from Step 0 config), run a passive monitoring scan to detect changes since the last session.
...
**First, check config:** If `adaptive_learning.integration.suggest_on_session_start` is `false`, display:
```
### Pending Suggestions
...
Run `/gsd:suggest` to review and accept/dismiss suggestions.
```
...
Use the Glob tool to find `SKILL.md` files in `.claude/skills/` and `~/.claude/skills/`:
```bash
# Count installed skill directories
ls ~/.claude/skills/ 2>/dev/null | wc -l
```

For each skill directory found, estimate its SKILL.md size. Calculate approximate total character count.

**Read thresholds from config** (from Step 0):
- `adaptive_learning.token_budget.max_percent` (default: 5)
- `adaptive_learning.token_budget.warn_at_percent` (default: 4)
...
If no skills are found, display: "No skills installed. Skills are installed to `~/.claude/skills/` by the GSD installer."
...
- Integration config is read from .planning/config.json under the adaptive_learning key and respected (feature toggles, budget thresholds)
