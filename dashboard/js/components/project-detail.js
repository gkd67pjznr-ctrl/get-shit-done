              ...
              <div class="phase-list">
                ${phases.map((p, i) => html`
                  <div class="phase-item ${p.status}" key=${i}>
                    ${p.number ? html`<span style="font-family:var(--font-data);font-size:11px;color:var(--text-muted);width:28px;flex-shrink:0">${p.number}</span>` : null}
                    <span>${p.name}</span>
                  </div>
                `)}
              </div>
              ...
              <div style="display:flex;flex-direction:column;gap:6px;font-size:13px;color:var(--text-secondary)">
                ...
                ${pct !== null ? html`
                  <div style="display:flex;align-items:center;gap:8px;margin-top:4px">
                    ...
                    <span style="font-family:var(--font-data);font-size:12px;color:var(--text-secondary)">${fmtPct(pct)}</span>
                  </div>
                ` : null}
              </div>
            </div>
  ...
  return html`
    <div class="detail-view">
      ...
      ${sorted.length === 0 ? html`
        <div style="color:var(--text-muted);font-size:14px">No milestone data available.</div>
      ` : null}
