  ...
  return html`
    <div class="sidebar-metrics">
      ...
      <div class="sidebar-metric-row">
        ...
        <span class="sidebar-metric-value" style="font-size:12px;color:var(--text-secondary)">
      ...
      <div class="sidebar-health-summary">
        ${healthCounts.healthy > 0 ? html`<span style="color:var(--term-green);font-family:var(--font-data);font-size:12px">${healthCounts.healthy} healthy</span>` : null}
        ${healthCounts.attention > 0 ? html`<span style="color:var(--signal-warning);font-family:var(--font-data);font-size:12px">${healthCounts.attention} attention</span>` : null}
        ${healthCounts.risk > 0 ? html`<span style="color:var(--term-red);font-family:var(--font-data);font-size:12px">${healthCounts.risk} at risk</span>` : null}
      </div>
    </div>
  `;
}
...
export function Sidebar({ onClose }) {
  ...
  if (ps.length === 0) {
    return html`<nav class="sidebar"><div style="padding:16px;font-size:14px;color:var(--text-muted)">No projects</div></nav>`;
  }
            ...
            ${activeMilestones.map(ms => {
              ...
              return html`
                <div class="sidebar-milestone ${statusClass(msStatus)}" key=${ms.name}
                  ...
                  ${ms.name}${total > 0 ? html` <span style="color:var(--term-dim);font-size:12px">[${done}/${total}]</span>` : null}
                </div>
              `;
            })}
          </div>
