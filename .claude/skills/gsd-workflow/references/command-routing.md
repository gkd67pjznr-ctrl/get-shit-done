...
| User Intent | GSD Command | Rationale |
|-------------|-------------|-----------|
| "work on tech debt" | `/gsd:fix-debt` | Guided workflow for selecting and resolving debt entries |
| "show me open debt" | `/gsd:fix-debt` | Displays open debt with impact scores |
| "log some tech debt" | `gsd-tools debt log` | Directly appends entry to DEBT.md |
| "list all debt entries" | `gsd-tools debt list` | Shows debt with optional status/type/severity filter |
| "mark debt as resolved" | `gsd-tools debt resolve` | Transitions status in-place |
| "which debt has most impact?" | `gsd-tools debt impact` | Ranks entries by correction-correlation count |
