...
At session start, relevant corrections and preferences are injected into context via the `gsd-recall-corrections` hook. Before writing code or making decisions, review any correction reminders from session start to avoid repeating known mistakes.

If no correction reminders appeared at session start, there is no action needed — the corrections store is empty or all items have been retired into skill refinements.
