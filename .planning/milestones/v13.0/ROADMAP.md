...
Three focused features that unify fragmented event streams, surface hidden context costs, and add advisory MCP routing intelligence. Phase 57 builds the event journaling backbone — a central emitter integrated across all emission points with a reader and digest surface. Phase 58 adds context budget tracking per skill so the most expensive low-relevance skills become visible candidates for deferral. Phase 59 closes with MCP server selection intelligence that classifies tasks before executor spawn and recommends appropriate servers.
...
- [x] **Phase 57: Event Journaling** - Central workflow.jsonl emitter, full integration, reader, digest timeline, and rotation
- [x] **Phase 58: Context Budget Optimizer** - Per-skill token cost measurement, session recording, aggregation, and digest surfacing
- [x] **Phase 59: MCP Server Selection** - Task-type classifier, recommendation mapping, execute-plan integration, and dashboard validation

## Phase Details

### Phase 57: Event Journaling
...
**Success Criteria** (what must be TRUE):
  ...
  3. Running /gsd:digest for a completed plan shows an event timeline ("Plan 57-01 timeline: N events across X minutes")
  4. After the entry count reaches the configured threshold, workflow.jsonl rotates to a backup and starts fresh
**Plans**: TBD

Plans:
- [x] 57-01: Implement emitEvent() core function and workflow.jsonl writer with rotation
- [x] 57-02: Integrate emitter into gate-runner, correction hooks, workflow observe steps, and session hooks; implement reader function with filter support; surface event timeline in /gsd:digest
- [x] 57-03: Add Event Timeline section to /gsd:digest (Step 3j); write 6 integration tests

### Phase 58: Context Budget Optimizer
...
Plans:
- [x] 58-01: Implement token cost measurement function, record skill_token_cost in sessions.jsonl, build aggregation function, and surface context budget analysis in /gsd:digest

### Phase 59: MCP Server Selection
...
Plans:
- [x] 59-01: Implement task-type classifier and MCP recommendation mapping
- [x] 59-02: Integrate recommendation emission into execute-plan workflow and add dashboard validation query
...
|-------|----------------|--------|-----------|
| 57. Event Journaling | 3/3 | Complete    | 2026-04-04 |
| 58. Context Budget Optimizer | 1/1 | Complete    | 2026-04-04 |
| 59. MCP Server Selection | 2/2 | Complete    | 2026-04-04 |
